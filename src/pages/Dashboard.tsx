...
const Dashboard = () => {
  ...
  const sidebarStyles = "fixed lg:static inset-y-0 left-0 z-50 w-64 bg-primary text-primary-foreground transform transition-transform duration-300 ease-in-out lg:translate-x-0";
  const cardStyles = "bg-white border-0 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] overflow-hidden relative";
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex font-body">
          ...
          <nav className="flex-1 px-4 space-y-1 mt-4 overflow-y-auto">
            {navItems.map((item) => (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-4 px-5 py-3.5 font-body text-sm rounded-2xl transition-all duration-300",
                  activeTab === item.id
                    ? "bg-white/20 text-white shadow-sm font-medium"
                    : "text-white/70 hover:bg-white/10 hover:text-white"
                )}
              >
                <item.icon className={cn("w-5 h-5", activeTab === item.id ? "opacity-100" : "opacity-70")} />
                {item.label}
              </button>
            ))}
          </nav>
      ...
      <main className="flex-1 min-w-0 flex flex-col max-h-screen overflow-y-auto">
        {/* Top bar */}
        <header className="sticky top-0 z-30 bg-[#FAFAFA]/80 backdrop-blur-xl px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full hidden sm:block">
              ...
              <input
                ...
                placeholder="Search..."
                className="w-full pl-12 pr-4 py-3 bg-white border-0 shadow-sm font-body text-sm rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-primary/30 text-primary"
              />
            </div>
          </div>
          
          <div className="flex items-center gap-5">
            <button className="relative text-primary/60 hover:text-primary transition-colors hover:bg-primary/5 p-2 rounded-full">
              <Bell className="w-6 h-6" />
              <span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 border border-white rounded-full" />
            </button>
            <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-primary to-primary/60 p-[2px] cursor-pointer shadow-md">
              <div className="h-full w-full bg-white rounded-full flex items-center justify-center text-primary font-display font-bold">
          ...
          >
            ...
            {activeTab === "overview" && (
              <div className="space-y-6 pb-10">
                <div className="mb-6">
                  <h1 className="font-display text-[2rem] font-bold tracking-tight text-charcoal mb-1">Dashboard</h1>
                  <p className="text-primary/60 font-body text-sm">Welcome back! Here's your salon overview.</p>
                </div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className={cardStyles + " p-6"}>
                    ...
                    <p className="font-display text-2xl font-bold text-charcoal tracking-tight">{allClients.length}</p>
                    <p className="text-xs font-body text-primary/60 mt-1">Total Customers</p>
                  </div>
                  ...
                  <div className={cardStyles + " p-6"}>
                    ...
                    <p className="font-display text-2xl font-bold text-charcoal tracking-tight">{todaysAptCount}</p>
                    <p className="text-xs font-body text-primary/60 mt-1">Today's Appointments</p>
                  </div>
                  ...
                  <div className={cardStyles + " p-6"}>
                    ...
                    <p className="font-display text-2xl font-bold text-charcoal tracking-tight">${todayRevenue}</p>
                    <p className="text-xs font-body text-primary/60 mt-1">Today's Revenue</p>
                  </div>
                  ...
                  <div className={cardStyles + " p-6"}>
                    ...
                    <p className="font-display text-2xl font-bold text-charcoal tracking-tight">{dbServices.length}</p>
                    <p className="text-xs font-body text-primary/60 mt-1">Active Services</p>
                  </div>
                </div>

                <div className="grid lg:grid-cols-3 gap-4">
                  <div className={cardStyles + " lg:col-span-2 p-6"}>
                    <h2 className="font-display text-xl font-bold text-charcoal mb-6">Revenue Trend</h2>
                    <div className="h-[280px] w-full mt-4">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={revenueData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                          <XAxis 
                            dataKey="name" 
                            axisLine={{ stroke: 'hsl(var(--primary)/20%)' }}
                            tickLine={false} 
                            tick={{ fill: 'hsl(var(--primary))', fontSize: 12, opacity: 0 }} // Hidden ticks to match design exactly
                            dy={10}
                          />
                          <YAxis 
                            axisLine={{ stroke: 'hsl(var(--primary)/20%)' }} 
                            tickLine={false} 
                            tick={{ fill: 'hsl(var(--primary))', fontSize: 12, opacity: 0.5 }}
                            dx={-10}
                            ticks={[3000, 6000, 9000, 12000]}
                            domain={[0, 15000]}
                          />
                          <Tooltip 
                            contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', fontFamily: 'DM Sans', fontWeight: 600, color: 'hsl(var(--primary))' }}
                          />
                          <Line 
                            type="monotone" 
                            dataKey="value" 
                  ...
                  <div className={cardStyles + " p-6 flex flex-col items-center"}>
                    <h2 className="font-display text-xl font-bold text-charcoal w-full mb-6 text-left">Service Popularity</h2>
                        ...
                        <PieChart>
                          {serviceData.length > 0 ? (
                            ...
                            >
                             ...
                             // Empty state pie
                             <Pie data={[{value: 1, color: '#F5E6E8'}]} cx="50%" cy="50%" innerRadius={70} outerRadius={90} dataKey="value" stroke="none">
                                <Cell fill="#F5E6E8" />
                             </Pie>
                          )}
                          <Tooltip 
                            contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', fontFamily: 'DM Sans' }}
                          />
                        </PieChart>
                      </ResponsiveContainer>
                    </div>
                    {/* Optional: Legend below pie chart */}
                    {serviceData.length > 0 && (
                      <div className="mt-4 flex flex-wrap justify-center gap-4">
                        {serviceData.map((item, i) => (
                          <div key={i} className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }} />
                            <span className="text-xs font-body text-[#2C2424]/70">{item.name}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            {activeTab === "finance" && (
              <div className="space-y-10 pb-16">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="bg-primary hover:bg-primary/90 text-white px-8 py-3.5 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg shadow-primary/20 active:scale-95"
                    >
                      <Plus className="w-5 h-5" /> Add Payment
                    </button>
                    <button className="bg-white hover:bg-zinc-50 text-zinc-600 border border-zinc-200 px-8 py-3.5 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all shadow-sm active:scale-95">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    { label: 'Revenue Today', value: financeStats[0], icon: Clock, color: 'from-emerald-400 to-emerald-600', shadow: 'shadow-emerald-500/10' },
                    { label: 'Weekly Income', value: financeStats[1], icon: TrendingUp, color: 'from-blue-400 to-blue-600', shadow: 'shadow-blue-500/10' },
                    { label: 'Monthly Earnings', value: financeStats[2], icon: Calendar, color: 'from-primary to-accent', shadow: 'shadow-primary/10' },
                    { label: 'Net Revenue', value: financeStats[3], icon: DollarSign, color: 'from-zinc-700 to-zinc-900', shadow: 'shadow-zinc-900/10' }
                  ].map((period, i) => (
                    <motion.div 
                      key={period.label}
                      ...
                      className={cn(
                        "bg-white p-6 rounded-[2.5rem] border border-zinc-100 relative overflow-hidden group hover:border-primary/20 transition-all duration-500",
                        period.shadow
                      )}
                    >
                      <div className="flex items-center justify-between mb-4 relative z-10">
                        <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br text-white shadow-lg transform group-hover:rotate-6 transition-transform", period.color)}>
                          <period.icon className="w-6 h-6" />
                        </div>
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">{period.label.split(' ')[1]}</span>
                      </div>
                      <div className="relative z-10">
                        <h3 className="font-display text-4xl font-bold text-zinc-900 tracking-tighter mb-1">${period.value.toLocaleString()}</h3>
                        <p className="text-xs font-medium text-zinc-500 uppercase tracking-widest">{period.label}</p>
                      </div>
                      <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-zinc-50 rounded-full group-hover:scale-150 transition-transform duration-700 pointer-events-none opacity-50" />
                    </motion.div>
                  ))}
                </div>
